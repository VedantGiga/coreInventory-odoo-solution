...
import { ArrowUpFromLine, Plus, Search, CheckCircle2, FileText, Trash2 } from 'lucide-react';
// Reuse existing receipts styling or global styling
import './Receipts.css'; 
...
export function Deliveries() {
  ...
  const [toLocationId, setToLocationId] = useState('loc-customer');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (lines.length === 0) {
      alert("Please add at least one product to deliver.");
      return;
    }
  ...
  const handleValidate = (id: string) => {
    if (window.confirm("Are you sure you want to validate this delivery? This will reduce stock levels immediately.")) {
      validateMovement(id);
    }
  };
    ...
    <div className="page-container flex-col gap-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted mt-1">Manage outgoing shipments to customers</p>
        </div>
              ...
              {deliveries.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center text-muted" style={{ padding: '3rem' }}>
                     No delivery orders found. Create a new delivery to get started.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={delivery.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded bg-danger-light flex justify-center items-center text-danger">
                          <ArrowUpFromLine size={20} />
                        </div>
                        <div className="flex-col">
                          <span className="font-semibold">{delivery.reference}</span>
                          <span className="text-xs text-muted">{delivery.lines.length} items</span>
                        </div>
                      </div>
                    </td>
                    <td>{getLocationName(delivery.fromLocationId)}</td>
                    <td>{getLocationName(delivery.toLocationId)}</td>
                    <td>{new Date(delivery.date).toLocaleDateString()}</td>
                    <td>
                      <span className={`badge badge-${delivery.status === 'DONE' ? 'success' : 'warning'}`}>
                        {delivery.status}
                      </span>
                    </td>
                    <td>
                      <div className="flex justify-end gap-2">
                        ...
                        {delivery.status !== 'DONE' && (
                          <button className="btn btn-primary text-xs" onClick={() => handleValidate(delivery.id)}>
          ...
          <div className="modal-content card flex-col gap-6 animate-fade-in" style={{ maxWidth: '800px' }}>
            ...
            <form onSubmit={handleSubmit} className="flex-col gap-6">
              <div className="grid gap-4" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                ...
                <div className="form-group flex-col">
                  <label className="label">Deliver To (Customer) *</label>
              ...
              <div className="grid gap-4" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
                ...
                <div className="form-group flex-col">
                  ...
                  <input type="text" className="input-field" placeholder="SO-2023-001" value={notes} onChange={e => setNotes(e.target.value)} />
                </div>
              </div>
              ...
              <div className="lines-section flex-col gap-3 mt-2">
                ...
                {lines.length === 0 ? (
                  <div className="text-center p-4 text-muted text-sm border border-dashed rounded" style={{ borderColor: 'var(--border)' }}>
                    Click "+ Add Line" to add products to this delivery order.
                  </div>
                ) : (
              ...
              <div className="flex justify-end gap-3 pt-4 border-t mt-4" style={{ borderColor: 'var(--border)' }}>
                ...
                <button type="submit" className="btn btn-primary">Save Delivery Order</button>
              </div>
            </form>
          </div>
        </div>
